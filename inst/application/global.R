...
resumen.numerico <- function(data, variable){
  ...
  datos.numericos <- list(Q1 = list(id = "q1", Label = "Primer Cuartil",
                                    Value = format(round(quantile(
                                      data[, variable], .25), 3), scientific = F),
                                    color = "green"),
                          Mediana = list(id = "mediana", Label = "Mediana",
                                         Value = format(round(median(
                                           data[, variable]), 3), scientific = F),
                                         color = "orange"),
                          Q3 = list(id = "q3", Label = "Tercer Cuartil",
                                    Value = format(round(quantile(
                                      data[, variable], .75), 3), scientific = F),
                                    color = "maroon"),
                          Minimo = list(id = "minimo", Label = "Mínimo",
                                        Value = format(round(min(
                                          data[, variable]), 3), scientific = F),
                                        color = "red"),
                          Promedio = list(id = "promedio", Label = "Promedio",
                                          Value = format(round(mean(
                                            data[, variable]), 3), scientific = F),
                                          color = "blue"),
                          Maximo = list(id = "maximo", Label = "Máximo",
                                        Value = format(round(max(
                                          data[, variable]), 3), scientific = F),
                                        color = "purple"),
                          DS <- list(id = "ds", Label = "Desviación Estandar",
                                     Value = format(round(sd(
                                       data[, variable]), 3), scientific = FALSE,
                                       nsmall = 3),
                                     color = "yellow"))

  for (calculo in datos.numericos) {
    salida <- paste0(salida, "<div class='shiny-html-output col-sm-6 shiny-bound-output' id='",
...
default.calc.normal <- function(data = "datos"){
  return(paste0(
    "calc <- lapply(var.numericas(datos), function(i) modeest::skewness(i)[1]) \n",
...
def.code.num <- function(data = "datos", variable = "input$sel.distribucion",
...
cluster.radar <- function(esHC = T, colores = "'steelblue'"){
  ...
  return(paste0(code.centros, "\ncentros.radar(centros) + \n",
                "  scale_color_manual('Clústeres', values = c(",
                   paste(colores, collapse = ","), ")) +\n",
...
cluster.cat <- function(esHC = T, var, colores = "'steelblue'") {
  code.clusters <-
    ifelse(esHC, "hc.modelo$clusters", "as.factor(k.modelo$cluster)")
  return(paste0("NDatos <- cbind(datos, Cluster = ", code.clusters, ") \n",
                "plot(ggplot(NDatos, aes(x = ", var, ")) +",
